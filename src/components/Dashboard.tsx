...
export function Dashboard() {
  ...
  useEffect(() => {
    ...
    const unsubGoalDeposits = onGoalDepositsSnapshot((deposits) => setGoalDeposits(deposits));
    // User settings (one-time fetch is fine)
    getUserSettings().then(settings => {
      if (settings) {
        const nameParts = [settings.firstName, settings.lastName].filter(Boolean);
    ...
    <div className="p-6 space-y-6 min-h-screen bg-gradient-to-br from-background to-muted/20">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <AddTransactionDialog onTransactionAdded={handleRefreshTransactions} />
          <Button 
            variant="outline" 
            size="sm" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <CardContent>
            ...
            <div className="flex items-center mt-2">
              ...
              <span className="text-sm text-success">
                {lastMonth.savings >= 0 ? '+' : ''}{formatCurrency(lastMonth.savings, currency)} this month
              </span>
            </div>
          </CardContent>
        </Card>
        <Card className="dashboard-card stats-card group hover:scale-[1.02] transition-all duration-300">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-success">{formatCurrency(lastMonth.income, currency)}</div>
            <div className="flex items-center mt-2">
              <ArrowUpRight className="h-4 w-4 text-success mr-1" />
              <span className="text-sm text-success">{prevMonth.income > 0 ? ((lastMonth.income - prevMonth.income) / prevMonth.income * 100).toFixed(1) : '0.0'}% from last month</span>
            </div>
          </CardContent>
        </Card>
        <Card className="dashboard-card stats-card group hover:scale-[1.02] transition-all duration-300">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-destructive">{formatCurrency(lastMonth.expenses, currency)}</div>
            <div className="flex items-center mt-2">
              <ArrowDownRight className="h-4 w-4 text-destructive mr-1" />
              <span className="text-sm text-destructive">{prevMonth.expenses > 0 ? ((lastMonth.expenses - prevMonth.expenses) / prevMonth.expenses * 100).toFixed(1) : '0.0'}% from last month</span>
            </div>
          </CardContent>
        </Card>
        <Card className="dashboard-card stats-card group hover:scale-[1.02] transition-all duration-300">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-warning">{formatCurrency(currentSavings, currency)}</div>
            <div className="w-full bg-muted/30 rounded-full h-2 mt-2">
              <div className="bg-gradient-gold h-2 rounded-full" style={{ width: `${Math.min(100, (currentSavings / savingsGoal) * 100)}%` }} />
            </div>
            <p className="text-sm text-muted-foreground mt-1">{Math.min(100, (currentSavings / savingsGoal) * 100).toFixed(1)}% of {formatCurrency(savingsGoal, currency)} goal</p>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="flex flex-wrap gap-3 justify-center mt-4">
              {expenseCategories.map((entry, idx) => (
                <div key={entry.name} className="flex items-center gap-2 text-xs">
                  ...
                  <span>{entry.name} {entry.percentage ? `${entry.percentage}%` : ''}</span>
                </div>
              ))}
            </div>
          </CardContent>
