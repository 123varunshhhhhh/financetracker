...
export function useCurrencyConversion() {
  ...
  const [currencyData, setCurrencyData] = useLocalStorage<CurrencyData | null>(
    "fintracker-currency-data",
    null
  );
  ...
  const handleCurrencyChange = useCallback(
    async (newCurrency: string, oldCurrency: string) => {
...
export function useTransactionConversion(
  ...
  const [convertedTransactions, setConvertedTransactions] =
  ...
  useEffect(() => {
    ...
    const needsConversion = transactions.some(
      (t) => t.originalCurrency && t.originalCurrency !== userCurrency
    );
